    ...
    try:
        ...
        while cap.isOpened():
            ...
            if results[0].boxes:
                ...
                for box in results[0].boxes:
                    ...
                    print(
                        f"   - {class_name:15} | Confidence: {confidence:.2f} | Box: ({x1:.0f}, {y1:.0f}, {x2:.0f}, {y2:.0f})"
                    )
        ...
        if detections_summary:
            ...
            for class_name, stats in sorted(
                detections_summary.items(), key=lambda x: x[1]["count"], reverse=True
            ):
                avg_conf = stats["avg_conf"] / stats["count"]
                print(
                    f"   {class_name:15} | Count: {stats['count']:3d} | Avg Confidence: {avg_conf:.2f}"
                )
        else:
